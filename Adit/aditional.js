...
document.getElementById('confirmDateBtn').addEventListener('click', () => {
    ...
    const command = 'getAdditationTransactionsByInterval';
    ...
    try {
        ...
        .then(data => {
            ...
            if (tot === 0) {
                alert("There is no datas");
                return;
            }
            document.querySelector('.total-price').textContent = tot;
...
document.getElementById('add_table_button').addEventListener('click', async () => {
    ...
    const host = localStorage.getItem('host');
    const port = localStorage.getItem('port');
    const command = "addAdditationTransaction";
    const URL = `http://${host}:${port}/${command}`;
...
async function handleSave(event) {
    ...
    const host = localStorage.getItem('host');
    const port = localStorage.getItem('port');
    const command = "updateAdditationTransaction";
    const URL = `http://${host}:${port}/${command}`;
...
async function handleRemove(event) {
    ...
    const host = localStorage.getItem('host');
    const port = localStorage.getItem('port');
    const command = "deleteAdditationTransaction";
    const URL = `http://${host}:${port}/${command}`;
